...
The revolutionary thing about Skills is that they are infinitely more flexible than Custom GPTs and also significantly more reliable.\[2\]
...
You can read all about Skills on [Agent Skills](https://agentskills.io/). The official line is that Skills enable:
...
This may or may not mean a lot to you but looking at the [example Skills](https://github.com/anthropics/skills/tree/main/skills) Anthropic shared on their GitHub should clear things up. The one that made the lightbulb go off over my head was the [PowerPoint skill](https://github.com/anthropics/skills/tree/main/skills/pptx). I’ve long wanted an LLM to make slideshows for me but they were always very unreliable. The PowerPoint file format is complicated and while the LLM can write it, it’s fraught with possibility for errors. This is one of the instruction in that `SKILL.md` file:
...
Another useful Skill to look at is the [Skill Creator](https://github.com/anthropics/skills/tree/main/skills/skill-creator) skill. Claude or another LLM can use to create more skills in the right format. The `SKILL.md` file of the Skill Creator contains little gems like this:
...
Now, what you can use Skills for? Anything you would like to use an LLM for in a systematic way. People have used it to help with marketing campaigns, research in a particular area, or even more crazy complicated things like I just did with [this skill](https://github.com/techczech/PPT2HandoutSkill) that creates a handout website based on a set of PowerPoint slides - like [this one](https://cogrevteach.pages.dev/). That skill has scripts that unpack the .pptx file and send images in it to Gemini 3 Flash (or a local model) and analyze them. I also have a skill that converts my rough notes into slides using my preferred style including SmartArt.
...
I’ve also made a public skill based on [my research into readability](https://readability.edutools.fyi/) to help people [create readable text](https://github.com/techczech/readability-skill).
...
The LLM is basically a blob of numbers on a server somewhere that some software uses to calculate the next word (or more accurately token) to add to the end of any text that it is given. Some people say this means it just “fills in the blank” or is really “just a glorified autocomplete”. Yes, that is sort of technically accurate but it obscures more than it reveals. The word “autocomplete” implies that this is a dumb and mechanical process - we all know how bad the next word prediction is on our phone. That is because the autocompletion on your phone is based on word frequencies.\[5\]
...
So that may make you feel safer but it comes with a downside. The downside that is at the very heart of the [Schemas and Propositions](https://schemasandpropositions.substack.com/) blog. How do we give compress knowledge accumulated over hours or days of interaction into a limited amount of text that fits in a context window?
...
Instead, the system running the LLM keeps track of the context window and when it’s close to filling up, it starts a process that will ask the LLM to summarize the context so far, throw everything else away (or move it to a file) and continue the process just based on that summary.\[6\]
...
Skills don’t exactly solve the context window\[7\] problem but they offer a very elegant and effective workaround. Using the tool calling and instruction following capabilities of the LLM, they simply rely on the LLM to be able to read the short notice that the skill is available in the system prompt, write a tool call to open the skill file\[8\], read the contents of the `SKILL.md` file and follow instructions. The `SKILL.md` may in turn instruct the model to make a series of tool calls and run scripts as needed.
...
Using n-grams to predict the next word can take you surprisingly far but you will hit a wall and there’s nothing you can do to get through unless you inject meaning.

1.  Claude - using my Readability Skill - says I should drop this section. It’s probably right but I couldn’t bring myself to do it. Here’s a link to what Claude suggested I write instead.
    
2.  For example, a custom GPT can use Code Interpreter to execute some code but the model has to write the code anew every time which makes it less reliable or predictable. The `\scripts` part of Skills takes care of that. The code is always the same which makes it reliable. Also, the code runs on your machine which faster and opens up many more capabilities.
    
3.  System prompt, for those that don’t know, is a hidden prompt that is sent to the LLM along with every chat message you send - yes, every single one - you can see what that’s like in a tool I build called [Model Context Explorer](https://modelcontextexplorer.edutools.fyi/).
    
4.  The reason for this is that the longer the text in the context window is, the more chance of some confusion because some contradictory parts the user may have forgotten about but that the model “sees.”
    
5.  Stricly speaking these are frequencies of n-grams (sequences of n words) rather than individual words but this does not take into account the meaning of the words. So the the most likely next word for “walk in ...” is “the” (as in “walk in the room”) but it could also be “my” (as in “walk in my room”). This would be obvious if the preceding text was somethign like “There’s nothing more annoying when my parents walk in ... room” without asking.” “The” is likely because it is the most frequent word in the English language and “walk in the” is the most common n-gram completion for “walk in”.
    
6.  Some frontier LLMs have been trained to be aware of this limitation so that they can call the summary tool themselves when they feel it’s the right time.
    
7.  The solution talked about here is often called “continual learning”. Instead of the models being pre-trained once and then again when a version comes, they would learn continually as users use them.
    
8.  It’s always important to remember that the model writing a tool call does nothing on its own. The app which sends the prompt to the model and receives the response needs to be able to parse the response and then trigger the right commands and execute them. This execution may happen inside the app or it may be passed onto the operating system of the computer or onto the internet to some external server.
