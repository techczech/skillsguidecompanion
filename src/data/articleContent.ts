...
export const articleSections: ArticleSection[] = [
  {
    ...
    title: 'Introduction',
    order: 1,
    content: `How a folder with some text files can move your use of AI from chatting with a bot to getting things done and what you can learn about how LLMs work in the process.
...
- want to understand how LLMs actually achieve what they do`,
  },
  {
    ...
    order: 2,
    content: `This article has two points and it takes its time to get to either of them. But I tried to make it contain all the things I think people most get wrong about AI today and also give some useful tips along the way.
...
When people figured out that's what it was, everyone became MCP-pilled (as the kids call it) and before we knew it MCP was supported by Anthropic's biggest competitors OpenAI and Google and now MCP is an open standard with these big companies and others working together to maintain and improve the standard.`,
  ...
  {
    id: 'skills-arrive',
    title: 'Skills arrive and completely change everything',
    order: 4,
...
But here's the good news. If you don't quite understand how LLMs work (and you're not alone in this), understanding Skills will help you understand much more about them and especially about the capabilities and limitations of the best LLMs as of early 2026 (and probably early 2027 as well - they will get much better but the foundations will probably remain the same).`,
  },
  {
    ...
    order: 5,
    content: `Ok, now that I've built it up, you're going to expect something spectacular, but no, Skills, just like MCP before them, are incredibly simple. Because in a way, LLMs are simple. MCP made things simpler and so do Skills. This is what a Skill is:
...
Want to share a Skill with somebody else? Easy, just send them a .zip file with the folder with all the Skill files in it. (Or put it on GitHub or anywhere else).`,
  ...
  {
    id: 'how-llm-uses-skills',
    title: 'How does the LLM in a chatbot use the Skill(s)?',
    order: 6,
...
And that's it.`,
  ...
  {
    id: 'what-skills-for',
    title: 'What can you use Skills for?',
    order: 7,
    content: `You can read all about Skills on [Agent Skills](https://agentskills.io/). The official line is that Skills enable:
...
I've also made a public skill based on [my research into readability](https://readability.edutools.fyi/) to help people [create readable text](https://github.com/techczech/readability-skill).`,
  ...
  {
    id: 'where-use-skills',
    title: 'Where can you use Skills?',
    order: 8,
    content: `At the moment, the only major chatbot that supports Skills is Anthropic's Claude. But all the big coding agents do. The AgentSkills website lists these coding tools from Anthropic's big competitors:
...
So we can expect that at least OpenAI will include them in the desktop version of ChatGPT. Gemini only works on the web and skills make a lot less sense there because of their reliance on local files and scripts (but that could certainly be overcome).`,
  ...
  {
    id: 'skills-teach-llm-capabilities',
    title: 'What do Skills teach us about LLM capabilities',
    order: 9,
    content: `Skills are interesting in their own right but even if you don't use any of these tools, you should still pay attention because they explain a lot about how LLMs work and what they can and cannot do.
...
### 1. Models can only read and output text (but they also "understand" it)
...
Sometimes the model can happily follow the instructions and send tool calls back and forth for 20 or more minutes and people have even had it run for hours or days. (Note: Some of that time may be spent by the model waiting for the results of the tool call - which can be even an hour if you're uploading a large website over a slow connection.)`,
  ...
  {
    id: 'llm-limitations',
    title: 'What limitations of LLMs do Skills solve',
    order: 12,
    content: `But Skills don't just take advantage of the models' capabilities, they also help solve of their limitations.
...
So that may make you feel safer but it comes with a downside. How do we compress knowledge accumulated over hours or days of interaction into a limited amount of text that fits in a **context window**?

Most LLMs that run agentic applications have context windows that can fit about 200 thousand tokens (or about 150,000 English words). That may sound like a lot but as you could see the tool calls take up a lot of space. And they add up quickly. You can easily fill up the context window in an hour of working (or less).
